...
type StudioTab = "editor" | "dashboard" | "validation" | "graph" | "llm" | "preview";
...
function AuthoringStudioInner({ onClose }: { onClose: () => void }) {
  ...
  return <div className="authoring-studio"><header className="authoring-top"><div><p className="eyebrow">Authoring Studio</p><h1>Via Prima Content QA</h1></div><div className="authoring-actions"><VpButton variant="ghost" icon={<ShieldCheck size={17} />} onClick={() => void store.validateDraft()} disabled={!store.selectedDocument}>Validate</VpButton><VpButton variant="ghost" icon={<Play size={17} />} onClick={() => void store.previewDraft()} disabled={store.selectedDocument?.kind !== "scene"}>Preview</VpButton><VpButton variant="ghost" icon={<GitBranch size={17} />} onClick={() => setTab("graph")}>Visual Graph</VpButton><VpButton variant="ghost" icon={<Sparkle size={17} />} onClick={() => setTab("llm")}>LLM Draft</VpButton><VpButton variant="ghost" icon={<Copy size={17} />} disabled={!store.selectedDocument} onClick={() => { const id = window.prompt("New id"); if (id) void store.duplicateDocument(id); }}>Duplicate</VpButton><VpButton variant="danger" icon={<Trash size={17} />} disabled={!store.selectedDocument} onClick={() => window.confirm("Delete document?") && void store.deleteDocument()}>Delete</VpButton><VpButton icon={<FloppyDisk size={17} />} disabled={!store.selectedDocument || store.saving} onClick={() => void store.saveDraft()}>Save</VpButton><VpButton variant="ghost" icon={<X size={17} />} onClick={onClose}>Close</VpButton></div></header><VpTabs ariaLabel="Authoring sections" active={tab} onChange={setTab} tabs={[{ id: "editor", label: "Editor" }, { id: "dashboard", label: "Dashboard" }, { id: "validation", label: "Validation" }, { id: "graph", label: "Visual Graph" }, { id: "llm", label: "LLM Draft Lab" }, { id: "preview", label: "Preview" }]} className="authoring-main-tabs" /><UnsavedChangesBar visible={store.unsavedChanges} saving={store.saving} onSave={() => void store.saveDraft()} onValidate={() => void store.validateDraft()} />{store.error ? <div className="error-band"><span>{store.error}</span><button type="button" onClick={store.clearError}>Kapat</button></div> : null}<div className="authoring-grid"><aside><ContentTree /></aside><main>{tab === "dashboard" ? <AuthoringDashboard /> : tab === "validation" ? <ValidationDashboard /> : tab === "graph" ? <SceneGraphEditor /> : tab === "llm" ? <LlmDraftLab /> : tab === "preview" ? <PlaytestPreview /> : store.selectedDocument ? <><EditorHeader onOpenGraph={() => setTab("graph")} /><div>{editor}</div><JsonDiffViewer original={store.selectedDocument.data} draft={store.draftData} /></> : <VpCard><VpEmptyState title="Belge sec">Sol agactan bir content belgesi ac.</VpEmptyState></VpCard>}</main><aside><FieldInspector document={store.selectedDocument} data={store.draftData} /><ValidationIssueList validation={store.validation} /><PlaytestPreview /></aside></div></div>;
}
